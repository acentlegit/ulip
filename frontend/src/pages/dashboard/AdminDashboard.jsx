  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStats({
        ...
        totalCases: res.data.cases || 0,
        totalClients: res.data.clients || 0,
        totalRevenue: res.data.revenue || 0,
        pendingTasks: res.data.tasks || 0,
        activeInvoices: res.data.invoices || 0,
      });
      setError("");
    } catch (error) {
  ...
  return (
    <div style={styles.container}>
      ...
      <div style={styles.actionsGrid}>
        <QuickAction title="Manage Users" icon="👥" link="/users" />
        <QuickAction title="View Reports" icon="📊" link="/reports" />
        <QuickAction title="System Settings" icon="⚙️" link="/admin/settings" />
        <QuickAction title="Activity Logs" icon="📋" link="/admin/logs" />
      </div>
    </div>
  );
}
