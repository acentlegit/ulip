  ...
  return (
    <div style={styles.container}>
      <div style={styles.header}>
        <h1 style={styles.title}>Paralegal Dashboard</h1>
        <p style={styles.subtitle}>Welcome back, {user?.name}</p>
      </div>

      <div style={styles.statsGrid}>
        <StatCard title="Assigned Cases" value={stats.myCases?.length || 0} icon="📁" color={theme.colors.primary} link="/cases" />
        <StatCard title="My Tasks" value={stats.myTasks?.length || 0} icon="✓" color={theme.colors.warning} link="/tasks" />
        <StatCard title="Hours This Month" value={stats.myHours?.toFixed(1) || 0} icon="⏱️" color={theme.colors.secondary} link="/time-tracking" />
      </div>
...
const styles = {
  ...
  list: { display: "flex", flexDirection: "column", gap: theme.spacing.md, marginBottom: theme.spacing.lg },
  ...
  listItemBadge: { padding: `${theme.spacing.xs} ${theme.spacing.sm}`, background: theme.colors.primary, color: theme.colors.white, borderRadius: theme.borderRadius.full, fontSize: theme.typography.fontSize.xs, fontWeight: theme.typography.fontWeight.medium },
  viewAll: { color: theme.colors.primary, textDecoration: "none", fontWeight: theme.typography.fontWeight.medium, fontSize: theme.typography.fontSize.sm },
};
